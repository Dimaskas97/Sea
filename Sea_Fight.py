...
deskSize = 10
ds1 = deskSize - 1
numLst = "A B C D E F G H I J".split()
life = 11
...
class Desk:
    ...
    def play(self):
        while True:
            print("Ваш ход. Передайте координаты для выстрела.")
...
class Ship:
    def __init__(self, xcords, ycords):
        self.size = len(ycords)
        self.health = self.size
        self.xcords = copy(xcords)
        self.ycords = copy(ycords)
...
me = Player(input("Ваше имя? "), bool(input("Подбрасываем монетку: True or False? ")))
comp = Player()
desk = Desk(me,comp)
desk.play()
print()
